...
<script type="text/javascript">
    ...
    function onSearch(currPage) {
        ...
        $.ajax({
            ...
            url: "<%=request.getContextPath()%>/cash/search"
        }).done(function (data) {
            $("#divTable").html(data);
    ...
    function prepareAdd(type) {
        if (type == 0) {
            $('#lblTitle').text('Thu tiền mặt');
        } else {
            $('#lblTitle').text('Chi tiền mặt');
        }
        $("#add-id").val('');
        $("#add-total").val('');
        $('#add-note').val('');
...
//                add["employeeId"] = null;
        ...
        $.ajax({
            ...
            url: "<%=request.getContextPath()%>/cash/add",
    ...
    function onDelete(id) {
        var content = "";
        content = 'Bạn chắc chắn muốn xóa chứng từ tiền mặt này?';
    ...
    function prepareDetail(status, id, customerId, total, note)
    {
        if (status == 0) {
            $('#lblTitle').text('Thu tiền mặt');
        } else {
    ...
    function onExport() {
        ...
        location.href = "<%=request.getContextPath()%>/cash/exportExcel?from=" + from + "&to=" + to;
    }
